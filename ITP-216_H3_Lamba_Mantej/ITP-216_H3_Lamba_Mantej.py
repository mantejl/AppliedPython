...
# Section: 31883R
# Assignment 3
# Description: assignment where we work with reading from files, and adding to lists to print out certain criteria
...
def find_movies_above_score(movies: list[str], score: float):
    ...
    """
    # looping through the movies list passed as an argument and if the score is greater than what is given
    # adding a list of the movie criteria to the list created
    movieList = []
    for m in movies:
        if(m[1] > int(score)):
            movieList.append([int(m[0]), int(m[1]), m[2]])
    return movieList
